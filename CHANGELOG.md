...
- The close-time hook refuses to load its scanner from a relative or empty `ATG_ROOT` — that would put a working-directory-relative entry first on `sys.path`, the shadowing `-I` exists to prevent — and traces `comment-scan-unavailable` instead. Its git calls also pin `core.fsmonitor` off and pass `--no-textconv`, so a repository config cannot get a command of its own choosing run inside the hook. They also pass `--no-optional-locks` where git accepts it — `129`, git's generic usage error, rejects the whole command line, so the first such rejection drops the flag and retries rather than letting every git question fail as an empty scan.
