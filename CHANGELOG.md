...
- `record_review_outcome` tool: records what the final whole-plan review (`final_review`) or a human-adjudicated PR review (`review_now`) found, per task, for a plan run, and returns the tasks anti-tangent passed that the review found a critical or major problem in. Deterministic, no reviewer call; categories and severities only. `reviewer_model` and `implementer_models[].model` are capped at 100 characters with no control character; a longer or control-character-bearing value is rejected (`recorded: false`) rather than stored.
- gnome-topbar: `/ui/runs` page. The overview is grouped by anti-tangent tool × validator model (calls, verdict mix, latency, escape and unconfirmed-flag rates with n), then regression by review model, the configured model sets, and a runs list with per-task drill-down showing every call's model. A Mine / Team / per-user scope reads the team's shared records from Basic Memory; `ANTI_TANGENT_SHARE_STATS=1` on a daemon publishes its own runs there as content-free `at_run` notes (`share_project`, default `bm_project`). Team records are cached on disk and pulled hourly (`team_refresh_minutes`, default 60), reading only notes not yet cached, with a daily full pull and a **Refresh now** button for an immediate one.
