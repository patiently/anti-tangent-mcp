...
- `final_diff` / `final_diff_path`: generate the diff with git — `git diff <base>..HEAD > /abs/path/final.diff` — and pass `final_diff_path`. Within each `diff --git`-headered section, a hand-assembled diff (two files' hunks concatenated under one header, hunks that run backwards, or a hunk whose declared line count doesn't match its body) is rejected as malformed evidence before the review. A diff with no `diff --git` header anywhere is not checked this way — `git diff` always produces one — so generate the diff with git rather than assembling it by hand.
