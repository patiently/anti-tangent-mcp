...
- `controller_verified_references` (optional, v0.4.0+): paths, symbols, line anchors, commands, or adjacent patterns that the controller already verified before dispatch. At most 200 entries on `validate_task_spec` and `validate_plan`. The reviewer treats these as caller-supplied attestations and suppresses matching `unverifiable_codebase_claim` findings only by deterministic substring match; contradictions, missing acceptance criteria, and ambiguity still surface.
- `verification` (optional, v0.22.0+): the task's steps and verify commands, at most 50 entries of at most 2000 characters. The pre-task review checks each gate against the task's Non-goals, and the final `validate_completion` review uses them to tell a Non-goal violation a gate forced from ordinary scope drift.
...
The server also **writes** one kind of file: `code_write` with a `target_path`. The same argument holds — the calling agent already has `Write` and `Edit`, so the server acquires no capability the caller lacks — but `ANTI_TANGENT_PLAN_ROOTS` is now load-bearing for writes as well as reads. A write resolves the target's **parent** directory (the leaf may not exist yet, so it cannot be `EvalSymlinks`-resolved the way a read's full path can) and containment-checks that.

**`overwrite: false`** (the default) creates the leaf directly with `O_CREAT|O_EXCL|O_NOFOLLOW`: it refuses outright if anything already exists there, and refuses a symlink at the leaf rather than following it. **`overwrite: true`** writes a temporary file in the target's own directory and renames it over the target (`os.CreateTemp` → write → `Sync` → close → `Rename`), so every failure before that rename leaves the target byte-identical to what it held before the call — the previous direct `O_TRUNC` open did not have this property: a failed write could leave the caller with an empty or half-written file and no way back. The rename carries the target's file mode across, but it **replaces the target's inode**: a hard link to `target_path` now points at the pre-write content rather than the new content, and ownership is not preserved across the rename (only the mode is). Anyone relying on a hard-linked `target_path` needs to know the link breaks on overwrite.

**`target_path` is refused outright on Windows** — this is a refusal, not merely a documented gap. The reason is the leaf: on Unix, `O_NOFOLLOW` / `os.Lstat` refuse a symlink planted at the target's final path component between the containment check and the write, however that symlink got there. Windows exposes no equivalent primitive through Go's `syscall` package, so a reparse point planted at the leaf would be a **static, non-race bypass** on that platform — not the narrow timing window the read path has (below) — and nothing in this repo builds or tests on Windows to validate a guard against it, so shipping one would put a security property's weight on code no test has ever run. `code_write` without `target_path` still generates the code and returns it as `code`, so the tool degrades gracefully rather than failing outright: omit `target_path` on Windows and write the file yourself.

This is a real asymmetry with the **read** path (`validate_plan`'s `plan_path`/`context_paths`, `validate_completion`'s `final_diff_path`/`final_files`), not an oversight. A read's full path must already exist to be read, so `resolveFileInput` can `EvalSymlinks` it whole, containment-check the fully-resolved path, and then re-stat from the already-open handle — its Windows residual is a narrow TOCTOU race, not a standing bypass, which is why reads stay allowed on Windows (see "File-path inputs and the trust model" above) while writes are refused there.

Even on Unix, an **ancestor** directory (not the leaf) can be swapped for a symlink between the parent's `EvalSymlinks` and the write that follows it — the leaf guard above does not catch that, since `O_NOFOLLOW` / `os.Lstat` only ever inspect the path's final component. **Requirement: `ANTI_TANGENT_PLAN_ROOTS` must not contain a directory that is group- or world-writable by a user you do not trust.** A same-uid racer gains nothing from this window — same-uid is not a privilege boundary in POSIX, and such a process could write the file itself, or `ptrace` this one — but a **different-uid** racer who can write into a directory inside the configured roots gets a genuine escalation this code does not stop: winning the race redirects the write **outside** those roots, which writing the target directly cannot achieve. (The overwrite path is incidentally harder to exploit this way, since its rename source is a temp file inside the same resolved parent and the swap invalidates that source too, so it tends to fail closed rather than land outside — but that is a narrowing, not a guarantee, and the create path's `O_EXCL` bounds it separately, to creating a file where none exists rather than clobbering one. See `internal/mcpsrv/file_target.go`'s package comment for the full argument.)
...
- `validate_completion` — call before claiming done. Walks every AC and non-goal explicitly. Accepts an optional structured `codescene` argument (see "Companion tool: CodeScene MCP" above). In lightweight mode (empty `session_id`) it also accepts `plan_run_id` with `task_index` or `task_title`, so the task is counted in `plan_run_report`. Accepts `controller_rulings` in lightweight mode too, and rejects a diff git could not have produced before the reviewer call.
