...
- `validate_completion` — call before claiming done. Walks every AC and non-goal explicitly. Accepts an optional structured `codescene` argument (see "Companion tool: CodeScene MCP" above). In lightweight mode (empty `session_id`) it also accepts `plan_run_id` with `task_index` or `task_title`, so the task is counted in `plan_run_report`.
