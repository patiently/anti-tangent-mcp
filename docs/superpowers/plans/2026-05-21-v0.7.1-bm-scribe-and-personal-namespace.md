...
- `<TYPE-PROMPTS>`:
  ...
  Then add a sub-step: **Auto-pick ADR number.** Before Step 3a, query `basic-memory:search_notes` with prefix `<PROJECT>/decisions/`. Parse the leading `NNNN-` off each returned permalink, find the max, and set `NNNN = max + 1` (zero-padded to four digits). If no decisions exist, start at `0001`.
...
- `<USERNAME>` = `$BM_SCRIBE_USERNAME` (or `$USER` on POSIX). If unset, ask the user for their handle and remember it.
...
- `<USERNAME>` = `$BM_SCRIBE_USERNAME`, then `$USER` (POSIX). If both unset, ask the user for their handle and cache the answer for the session. If unset, ask.
...
Expected: the eleven planned commit subjects (from Tasks 1-11) appear in plan order (oldest first — Task 1's `chore(release): start v0.7.1 work` is the first line). The `--reverse` flag is required because plain `git log` defaults to reverse-chronological. Extra commits — if any — are allowed but should be surfaced to the controller; the eleven planned subjects must be present in order. If a planned subject is missing, the corresponding task did not commit and must be re-run.
