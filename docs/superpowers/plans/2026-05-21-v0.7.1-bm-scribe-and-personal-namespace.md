...
- `<TYPE-PROMPTS>`:
  ...
  Then re-order Step 2's sub-steps. For `create-decision`, Step 2 has THREE sub-steps in this order:

  1. **Resolve `<PROJECT>`** — from `$BM_SCRIBE_PROJECT` or the answer captured in Step 1.
  2. **Auto-pick ADR number** — query `basic-memory:search_notes` with prefix `<PROJECT>/decisions/`. Parse the leading `NNNN-` off each returned permalink, find the max, and set `NNNN = max + 1` (zero-padded to four digits). If no decisions exist, start at `0001`.
  3. **Construct the canonical permalink** — `<PROJECT>/decisions/<NNNN>-<slug>/main` with both `<PROJECT>` and `<NNNN>` resolved from sub-steps 1 and 2. The directory portion (passed to `write_note`) is the permalink with `/main` stripped.

  Sub-step 2 MUST run before sub-step 3 because the permalink string contains `<NNNN>`. This ordering is unique to `create-decision`; the other five creators have a simpler Step 2 (just resolve `<PROJECT>` and construct the permalink — no auto-pick).
