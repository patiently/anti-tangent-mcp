...
- `<USERNAME>` = `$BM_SCRIBE_USERNAME` (or `$USER`). If unset, ask.
...
- `<USERNAME>` = `$BM_SCRIBE_USERNAME` (or `$USER`).
...
Print the full note: frontmatter (just `title` and `tags` — skip `permalink` since it's evident from the request), then the body. Preserve markdown formatting.
````
