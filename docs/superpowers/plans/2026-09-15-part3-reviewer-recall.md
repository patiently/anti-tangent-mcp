...
**Criteria amended at execution (2026-09-16, by the maintainer).** The assessed run's transcripts and saved diffs were not on the machine that executed this plan, so the first criterion below could not be met as written. The maintainer chose, from four options, to build the fixtures from this repository instead. The criteria below stand with that substitution, and with this limit stated plainly: **a replay on substitute fixtures cannot establish the spec's field-recall claim.** It measures whether each prompt change alters this reviewer's behaviour on a comparable case, which is what decided §3.3. Re-running against the assessed run's fixtures, before the release, remains open.

**Acceptance Criteria:**
- [ ] A fixture directory outside the repository holds, at minimum: `stale-comments` (a diff that deletes a declaration, with comments naming it in a changed file outside the hunks, and `repo_root` at a checkout of the post-change commit), `gate-vs-non-goal` (a task whose Non-goals contradict its verification gate, with `verification` set), `state-set-guard` (a diff that removes states a guard still enumerates), and `final-pass` (a real passing diff, with no expectations). Fixtures from the assessed run when its transcripts are available; otherwise the substitutes above, with the limit recorded in the close note
