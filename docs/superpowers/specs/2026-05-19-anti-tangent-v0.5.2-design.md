...
- **Partial recovery — no-recovery fallback** (`truncatedEnvelope` for per-task, `truncatedPlanResult` for plan-level): per-task path needs a MANDATORY severity bump from `minor` → `major` on the synthetic `reviewer_response` finding. Today's per-task `truncatedEnvelope` (handlers.go:390–403) emits `Verdict: warn` with a single `minor` finding — under the new ladder, a single `minor` derives to `pass`, leaving the explicit `Verdict: warn` self-inconsistent with the findings. Bump to `major` so the ladder derives `warn` from it (≥1 major → warn), then either (a) let `FinalizeVerdict` derive the verdict so the assignment lines up, or (b) keep the explicit `Verdict: warn` and rely on derivation parity — both produce `warn`. The plan-level `truncatedPlanResult` (handlers.go:522–532) ALREADY emits the finding at `SeverityMajor`; confirm it remains `major` after the refactor (no bump needed) so the ladder produces `warn` consistently with the existing explicit verdict. NOT a hard rejection (still `warn`, not `fail`) — the caller can retry with a higher `max_tokens_override`.
...
- `internal/mcpsrv/handlers.go`: in each per-task handler (`ValidateTaskSpec`, `CheckProgress`, `ValidateCompletion`), reorder the clamp-insertion: prepend `cc.Clamp` (and the `validate_plan` `prependPlanClamp` equivalent) into `result.Findings` / `pr.PlanFindings` BEFORE invoking `FinalizeVerdict` / `FinalizePlanVerdict`, then build the envelope from the finalized result without a trailing `prependClamp(env, ...)` call. Bump the severity of `malformed_evidence` / `payload_too_large` / `session_not_found` synthetic findings to `critical`. Bump the severity of the per-task `truncatedEnvelope` (handlers.go:390–403) synthetic `reviewer_response` finding from `minor` to `major`. Confirm that the plan-level `truncatedPlanResult` (handlers.go:522–532) synthetic finding remains `major` after the refactor — it is already `major` today; this is a no-change confirmation, not a bump. The existing `prependClamp` / `prependPlanClamp` helpers may remain in use for hard-rejection envelopes only (where the envelope's `Verdict: fail` is already correct and finalization is not invoked) — confirm by grepping every call site after the refactor.
- `internal/mcpsrv/review_error.go`: `handlePerTaskReviewErr` and `handlePlanReviewErr` are where partial-recovery + clamp coordination lives today (`handlePerTaskReviewErr` builds an `Envelope` either from `recoverPartialFindings` or `truncatedEnvelope`, then calls `prependClamp`; `handlePlanReviewErr` analogously uses `truncatedPlanResult` + `prependPlanClamp`). The clamp-ordering refactor (clamp into result/findings BEFORE finalize) applies here too: when partial recovery succeeds and produces a `verdict.Result` / `verdict.PlanResult`, prepend the clamp into the result's findings BEFORE invoking `FinalizeVerdict` / `FinalizePlanVerdict`. When the no-recovery fallback fires, the `truncatedEnvelope` / `truncatedPlanResult` is also fed through the finalizer so its verdict derives from the (now-major) synthetic finding consistently with the rest of the ladder. Update test coverage for both partial-success and no-recovery paths.
