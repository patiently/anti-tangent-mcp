...
If you prefer nginx, the equivalent shape is `map $http_authorization $allowed { default 0; "Bearer <PER_DEV_TOKEN_1>" 1; ... }` plus `if ($allowed = 0) { return 401; }` plus `proxy_pass http://127.0.0.1:8000;` with `proxy_buffering off;` and `proxy_read_timeout 0;` (unbounded — symmetric to Caddy's `read_timeout 0`). The load-bearing knob is `proxy_read_timeout 0;` — the active streaming response stays open indefinitely under it, regardless of how `keepalive_timeout` is configured. For the client-facing leg, nginx's `keepalive_timeout` directive is asymmetric with Caddy: nginx requires a finite value here (the literal `0` means "send `Connection: close`", disabling keep-alive entirely — NOT "unbounded"), so set `keepalive_timeout 1d;` at the server scope for an effectively-unbounded value, or simply omit the directive to keep nginx's default 75s. (Both streamable-http and SSE need unbuffered passthrough; the settings apply identically to either transport.)
